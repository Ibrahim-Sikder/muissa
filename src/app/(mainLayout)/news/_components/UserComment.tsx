...
import { useGetAllUserQuery, useGetMeQuery } from '@/redux/api/userApi';

const UserComment = ({ id }: any) => {
    const pathname = usePathname();
    const token = getCookie("mui-token") || "";

    const { data: commentData, error, isLoading, refetch } = useGetSingleBlogQuery({ id, pollingInterval: 1000 });
        ...
        <div className="comment sectionMargin">
            ...
            <div className="grid grid-rows-1 gap-10">
                {commentData?.comments?.map((data: any) => (
                    <div key={data.id} className="flex flex-col justify-between gap-10">
                        ...
                        <div className="commentCard">
                            ...
                            {visibleReply === data.id && <ReplyComment />}
                           {
                            verifyUser?.role === 'admin' ? (
                                <div className="flex items-center justify-end gap-2 mt-2">
                                <Button
                                    sx={{ width: "70px", height: "35px" }}
                                    onClick={() => toggleReply(data.id)}
                                >
